                        ...
                        <div class="gallery">
                            <div class="xzoom-container">
                                <img
                                    class="xzoom"
                                    id="xzoom-default"
                                    src="{{\Illuminate\Support\Facades\Storage::url($item->galleries->first()->image)}}"
                                    xoriginal="{{\Illuminate\Support\Facades\Storage::url($item->galleries->first()->image)}}"
                                />
                                <div class="xzoom-thumbs">
                                    @foreach($item->galleries as $gallery)
                                    <a href="{{\Illuminate\Support\Facades\Storage::url($gallery->image)}}"
                                    ><img
                                            class="xzoom-gallery"
                                            width="128"
                                            src="{{\Illuminate\Support\Facades\Storage::url($gallery->image)}}"
                                            xpreview="{{\Illuminate\Support\Facades\Storage::url($gallery->image)}}"
                                        /></a>
                                        @endforeach
                                </div>
                            </div>
